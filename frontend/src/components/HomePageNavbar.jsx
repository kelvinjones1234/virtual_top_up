...
const HomePageNavbar = ({ homeMenuToggle, setHomeMenuToggle }) => {
	...
	return (
		<>
			<div
				className={`p-4 lg:px-0 flex justify-between fixed top-0 w-full transition-colors duration-200 ${
					isScrolled ? "bg-opacity-100 bg-gray-900" : "bg-transparent"
				}`}
			>
				<div className="left">
					<div className="logo text-white lg:pl-[6rem]">Atom</div>
				</div>
				<div className="right">
					<div className={`small-screen flex items-center sm:hidden`}>
						<div className="get-started">
							<button className="bg-[#1CCEFF] py-[.4rem] mr-9 px-4 text-[.9rem] rounded-xl font-[500]">
					...
					<div className="large-screen hidden lg:pr-[6rem]">
						<div className="button">
							<button className="text-[#1CCEFF] border py-[.5rem] mr-8 font-semibold px-4 text-[.9rem] rounded-xl">
								<Link to={"/authentication/register"}>Login</Link>
							</button>
						</div>
						<div className="button">
							<button className="bg-[#1CCEFF] py-[.5rem] font-semibold px-4 text-[.9rem] rounded-xl">
